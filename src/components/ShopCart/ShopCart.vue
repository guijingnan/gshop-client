      ...
      <transition name="move">
        <div class="shopcart-list"  v-show="listShow" >
          ...
          <div class="list-content" ref="foodList">
            <ul>
              <li class="food" v-for="(food,index) in shopFoods" :key="index">
  ...
  export default {
    ...
    computed:{
      ...
      listShow(){
        ...
        if(this.isShow){
          //BScroll对象会禁止原生的dom事件，所以只能创建一个BScroll对象
          //没次调用listShow()时都会创建BScroll，所以在创建BScroll时，要先判断
          //BScroll存不存在，如果不存在，就创建，然后保存，由于BScroll只有当内容高度
          //超过容器高度时2，才会发生滑动，所以要this.scroll.refresh()
          this.$nextTick(() => {
            if(!this.scroll) {
              // 在创建后: 保存对象
