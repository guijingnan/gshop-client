  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active:selectType===0}"  @click="setSelectType(0)">
        推荐<span class="count">{{positiveRating}}</span>
      </span>
      <span class="block" :class="{active:selectType===1}" @click="setSelectType(1)">
        吐槽<span class="count">{{negativeRating}}</span>
      </span>
    </div>
    <div class="switch " :class="{on:isComment}" @click="toggleIsComment">
...
<script>
  ...
  export default {
    ...
    computed:{
      //通过计算属性获取总的评论，好的评论，差的评论
      ...mapGetters(['ratingCount','positiveRating','negativeRating'])
    },
    methods:{
      //自定义事件,分发事件，由父组件去更新状态
      toggleIsComment(){
        this.$emit('toggleIsComment')
      },
